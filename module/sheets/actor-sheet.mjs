...
export class MEGSActorSheet extends ActorSheet {
    ...
    constructor(object, options) {
        ...
        const isUnlocked =
            this.actor.isOwner && !this.actor._stats.compendiumSource;
    ...
    getData() {
                    ...
                    if (owner.items) {
                        ...
                        owner.items.forEach((element) => {
                            if (element.type === MEGS.itemTypes.gadget) {
                                ...
                                if (
                                    element._id === context.system.linkedItemId
                                ) {
        ...
        context.showHeroPointCosts = game.settings.get(
            "megs",
            "showHeroPointCosts"
        );
    ...
    _prepareInitiative(context) {
        ...
        const initiativeExplanation = this._formatInitiativeExplanation(context);

        // set value on actor sheet object
        context.system.initiativeBonus.value = initiativeBonus; // works for sheet display
        context.system.initiativeBonus.text = initiativeExplanation;
    ...
    _formatInitiativeExplanation(context) {
        ...
        '        <td class="value total">' +  this._calculateInitiativeBonus(context) + '</td>' +
        '    </tr>';
        ...
        context.items.forEach((i) => {
            ...
            // Append to skills.
                ...
                if (i.system.aps === 0) {
                    i.unskilled = true;
                    i.linkedAPs =
                        this.object.system.attributes[i.system.link].value;
                } else {
                    i.unskilled = false;
                }
                i.subskills = [];
            ...
            // Append to gadgets; do not show if gadget is owned by another gadget
                ...
                i.rollable =
                    i.system.effectValue > 0 || i.system.actionValue > 0;
                gadgets.push(i);
            }
        });
    ...
    _onRoll(event) {
        ...
        if (targetActor) {
            if (dataset.type === MEGS.rollTypes.attribute) {
                opposingValue =
                    targetActor.system.attributes[dataset.key].value;
                resistanceValue = Utils.getResistanceValue(
                    dataset.key,
                    targetActor
                );
            } else if (
                ...
                if (dataset.link) {
                    opposingValue =
                        targetActor.system.attributes[dataset.link].value;
                    resistanceValue = Utils.getResistanceValue(
                        dataset.link,
                        targetActor
                    );
                    ...
                    if (gadget.system.attributes.str > 0) {
                        ...
                        if (actionValue === 0) {
                            if (gadget.system.attributes.infl > 0) {
                                ...
                                actionValue =
                                    this.object.system.attributes.infl;
                            }
                        }
                    }
                } else {
                    console.error(
                        "No gadget with ID " + dataset.gadgetid + " found"
                    );
                }
            } else {
                // TODO
        ...
        rollTables
            .roll(event, this.object.system.heroPoints.value)
            .then((response) => {});
    }
    ...
    _changeEditHeaderLink(sheetHeaderLinks) {
        const found = sheetHeaderLinks.find(
            (element) => element.label === "Sheet"
        );
