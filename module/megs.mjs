...
Hooks.once("init", function () {
    ...
    _loadData("systems/megs/assets/data/combatManeuvers.json")
        .then((response) => {
            console.log(
                `Received response for combat maneuvers data: ${response.status}`
            );
    ...
    _loadData("systems/megs/assets/data/motivations.json")
        .then((response) => {
            console.log(
                `Received response for motivations data: ${response.status}`
            );
            ...
            if (i.type === MEGS.itemTypes.skill) {
                if (i.name === skillName) {
                    return game.i18n.localize(
                        CONFIG.MEGS.attributes[i.system.link.toLowerCase()]
                    );
                }
            }
        }
...
Handlebars.registerHelper("getSkillDisplayName", function (skill) {
    ...
    if (
        ...
        console.error(skill.subskills); // TODO delete
        let subskillText = " (";
...
Handlebars.registerHelper("getGadgetDescription", function (gadget) {
    ...
    for (let attributeName in gadget.system.attributes) {
        if (
            ...
            if (attribute.value > 0) {
                ...
                description +=
                    attributeName.toUpperCase() + " " + attribute.value;
            }
        }
    }
    ...
    if (owner && owner.items) {
        ...
        for (let i of owner.items) {
            if (
                ...
                description +=
                    i.linkedSkill + " (" + i.name + ") " + i.system.aps;
            }
        }
    }
    ...
    if (gadget.system.reliability > 0) {
        ...
        description +=
            "R # " + CONFIG.reliabilityScores[gadget.system.reliability];
    }
...
Handlebars.registerHelper(
    "shouldShowRow",
    function (index, hasAttributes, options) {
...
Handlebars.registerHelper(
    "shouldShowGadgetAttributesDetails",
    function (hasAttributes, options) {
        if (
            hasAttributes?.physical ||
            hasAttributes?.mental ||
            hasAttributes?.mystical
        ) {
            return options.fn(this);
        }
        return options.inverse(this);
...
Handlebars.registerHelper(
    "getVehicleOwnerName",
    function (ownerId, characters) {
        return characters[ownerId] || "-";
    }
);

Handlebars.registerHelper(
    "getLinkedVehicleItemName",
    function (vehicleId, vehicles) {
...
Handlebars.registerHelper(
    "getMotivation",
    function (descriptionIndex, descriptions) {
...
Hooks.once("ready", function () {
    ...
    Hooks.on("chatMessage", (log, message, data) =>
        interceptMegsRoll(message, data)
    );
});
